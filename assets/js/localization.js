...
$(document).ready(function () {
    ...
    $.each(iso639Codes, function (code, language) {
...
function getLocales() {
    ...
    if(config.LOCALES) {
        ...
        if(locales) {
            ...
            console.warn('Available locales cache ' + (locales === null ? 'stale' : 'miss') + ',  retrieving from server');
            $.ajax({
...
function localizeLanguageNames(localizedNamesFromJSON) {
    if(config.LANGNAMES[locale]) {
        ...
        if(localizedNames) {
            ...
            console.warn(locale + ' localized names cache ' + (localizedNames === null ? 'stale' : 'miss') + ',  retrieving from server');
            $.jsonp({
    ...
    function handleLocalizedNames(localizedNames) {
        ...
        $.each(localizedNames, function (code, name) {
...
function localizeStrings(stringsFresh) {
    if(stringsFresh) {
        ...
        if(localizations) {
            ...
            console.warn(locale + ' localizations cache ' + (localizations === null ? 'stale' : 'miss') + ',  retrieving from server');
            $.ajax({
    ...
    function handleLocalizations(localizations) {
        for(var textId in localizations) {
            if(textId.charAt(0) !== '@') {
                ...
                $.each(config.REPLACEMENTS, function (name, replacement) {
